...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		//加载路径下的所有资源
		//在加载资源过程时，如果在构造 PathMatchingResourcePatternResolver 实例的时候如果传入了 ClassLoader
		//则调用该 ClassLoader 的 #getResources() 方法，否则调用 ClassLoader#getSystemResources(path) 方法
		...
		//加载路径下的所有jar包
		//若 path 为空（“”）时，则调用 #addAllClassLoaderJarRoots(...)方法。该方法主要是加载路径下得所有 jar 包
		if ("".equals(path)) {
	 ...
	 * 方法比较绕，效果如下示例：
	 * 原路径	确定根路径
	 * classpath*:test/cc*\/spring-mbc.xml  classpath*:test/
	...
	protected String determineRootDir(String location) {
		...
		//如果查找完成后，rootDirEnd=0了，则将之前赋值的prefixEnd的值赋给rootDirEnd，也就是冒号的后一位
		if (rootDirEnd == 0) {
